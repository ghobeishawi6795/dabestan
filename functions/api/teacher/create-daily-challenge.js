...
export async function onRequestPost({ request, env, data }) {
  ...
  try {
    await env.DB.prepare(
      `INSERT INTO daily_challenges (school_id, title, description, challenge_date, reward_xp)
       VALUES (?, ?, ?, ?, ?)
       ON CONFLICT (school_id, challenge_date) DO UPDATE SET title = excluded.title, description = excluded.description, reward_xp = excluded.reward_xp`
    ).bind(
