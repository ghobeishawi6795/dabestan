...
export async function onRequestGet({ env, data }) {
  ...
  const studentId = Number(
    new URL(data.request?.url || 'https://local')
      .searchParams.get('student_id') || 0
  );
  ...
  // دانش‌آموز را همراه با parent_code واقعی می‌گیریم
  ...
  // مالکیت واقعی:
  // فقط والدی که parent_code او با parent_code دانش‌آموز یکی است
  // اجازه دیدن گزارش را دارد.
